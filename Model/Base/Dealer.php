...
abstract class Dealer implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildDealerImage[] Collection to store aggregation of ChildDealerImage objects.
     */
    protected $collDealerImages;
    protected $collDealerImagesPartial;

    /**
     * @var        ObjectCollection|ChildDealerMetaSeo[] Collection to store aggregation of ChildDealerMetaSeo objects.
     */
    protected $collDealerMetaSeos;
    protected $collDealerMetaSeosPartial;
     ...
     * @var ObjectCollection
     */
    protected $dealerImagesScheduledForDeletion = null;

    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection
     */
    protected $dealerMetaSeosScheduledForDeletion = null;
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collDealerImages = null;

            $this->collDealerMetaSeos = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->dealerImagesScheduledForDeletion !== null) {
                if (!$this->dealerImagesScheduledForDeletion->isEmpty()) {
                    \Dealer\Model\DealerImageQuery::create()
                        ->filterByPrimaryKeys($this->dealerImagesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->dealerImagesScheduledForDeletion = null;
                }
            }

                if ($this->collDealerImages !== null) {
            foreach ($this->collDealerImages as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
            ...
            if ($this->dealerMetaSeosScheduledForDeletion !== null) {
                if (!$this->dealerMetaSeosScheduledForDeletion->isEmpty()) {
                    \Dealer\Model\DealerMetaSeoQuery::create()
                        ->filterByPrimaryKeys($this->dealerMetaSeosScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->dealerMetaSeosScheduledForDeletion = null;
                }
            }

                if ($this->collDealerMetaSeos !== null) {
            foreach ($this->collDealerMetaSeos as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collDealerImages) {
                $result['DealerImages'] = $this->collDealerImages->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
            if (null !== $this->collDealerMetaSeos) {
                $result['DealerMetaSeos'] = $this->collDealerMetaSeos->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getDealerImages() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addDealerImage($relObj->copy($deepCopy));
                }
            }

            foreach ($this->getDealerMetaSeos() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addDealerMetaSeo($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('DealerImage' == $relationName) {
            return $this->initDealerImages();
        }
        if ('DealerMetaSeo' == $relationName) {
            return $this->initDealerMetaSeos();
        }
    ...
    /**
     * Clears out the collDealerImages collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addDealerImages()
     */
    public function clearDealerImages()
    {
        $this->collDealerImages = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collDealerImages collection loaded partially.
     */
    public function resetPartialDealerImages($v = true)
    {
        $this->collDealerImagesPartial = $v;
    }

    /**
     * Initializes the collDealerImages collection.
     *
     * By default this just sets the collDealerImages collection to an empty array (like clearcollDealerImages());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initDealerImages($overrideExisting = true)
    {
        if (null !== $this->collDealerImages && !$overrideExisting) {
            return;
        }
        $this->collDealerImages = new ObjectCollection();
        $this->collDealerImages->setModel('\Dealer\Model\DealerImage');
    }

    /**
     * Gets an array of ChildDealerImage objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return Collection|ChildDealerImage[] List of ChildDealerImage objects
     * @throws PropelException
     */
    public function getDealerImages($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collDealerImagesPartial && !$this->isNew();
        if (null === $this->collDealerImages || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collDealerImages) {
                // return empty collection
                $this->initDealerImages();
            } else {
                $collDealerImages = ChildDealerImageQuery::create(null, $criteria)
                    ->filterByDealer($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collDealerImagesPartial && count($collDealerImages)) {
                        $this->initDealerImages(false);

                        foreach ($collDealerImages as $obj) {
                            if (false == $this->collDealerImages->contains($obj)) {
                                $this->collDealerImages->append($obj);
                            }
                        }

                        $this->collDealerImagesPartial = true;
                    }

                    reset($collDealerImages);

                    return $collDealerImages;
                }

                if ($partial && $this->collDealerImages) {
                    foreach ($this->collDealerImages as $obj) {
                        if ($obj->isNew()) {
                            $collDealerImages[] = $obj;
                        }
                    }
                }

                $this->collDealerImages = $collDealerImages;
                $this->collDealerImagesPartial = false;
            }
        }

        return $this->collDealerImages;
    }

    /**
     * Sets a collection of DealerImage objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $dealerImages A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildDealer The current object (for fluent API support)
     */
    public function setDealerImages(Collection $dealerImages, ConnectionInterface $con = null)
    {
        $dealerImagesToDelete = $this->getDealerImages(new Criteria(), $con)->diff($dealerImages);


        $this->dealerImagesScheduledForDeletion = $dealerImagesToDelete;

        foreach ($dealerImagesToDelete as $dealerImageRemoved) {
            $dealerImageRemoved->setDealer(null);
        }

        $this->collDealerImages = null;
        foreach ($dealerImages as $dealerImage) {
            $this->addDealerImage($dealerImage);
        }

        $this->collDealerImages = $dealerImages;
        $this->collDealerImagesPartial = false;
    ...
    /**
     * Returns the number of related DealerImage objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related DealerImage objects.
     * @throws PropelException
     */
    public function countDealerImages(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collDealerImagesPartial && !$this->isNew();
        if (null === $this->collDealerImages || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collDealerImages) {
            ...
            if ($partial && !$criteria) {
                return count($this->getDealerImages());
            }

            $query = ChildDealerImageQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collDealerImages);
    }

    /**
     * Method called to associate a ChildDealerImage object to this object
     * through the ChildDealerImage foreign key attribute.
     *
     * @param    ChildDealerImage $l ChildDealerImage
     * @return   \Dealer\Model\Dealer The current object (for fluent API support)
     */
    public function addDealerImage(ChildDealerImage $l)
    {
        if ($this->collDealerImages === null) {
            $this->initDealerImages();
            $this->collDealerImagesPartial = true;
        ...
        if (!in_array($l, $this->collDealerImages->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddDealerImage($l);
        }
    ...
    /**
     * @param DealerImage $dealerImage The dealerImage object to add.
     */
    protected function doAddDealerImage($dealerImage)
    {
        $this->collDealerImages[]= $dealerImage;
        $dealerImage->setDealer($this);
    }

    /**
     * @param  DealerImage $dealerImage The dealerImage object to remove.
     * @return ChildDealer The current object (for fluent API support)
     */
    public function removeDealerImage($dealerImage)
    {
        if ($this->getDealerImages()->contains($dealerImage)) {
            $this->collDealerImages->remove($this->collDealerImages->search($dealerImage));
            if (null === $this->dealerImagesScheduledForDeletion) {
                $this->dealerImagesScheduledForDeletion = clone $this->collDealerImages;
                $this->dealerImagesScheduledForDeletion->clear();
            }
            $this->dealerImagesScheduledForDeletion[]= clone $dealerImage;
            $dealerImage->setDealer(null);
        }
    ...
    /**
     * Clears out the collDealerMetaSeos collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addDealerMetaSeos()
     */
    public function clearDealerMetaSeos()
    {
        $this->collDealerMetaSeos = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collDealerMetaSeos collection loaded partially.
     */
    public function resetPartialDealerMetaSeos($v = true)
    {
        $this->collDealerMetaSeosPartial = $v;
    }

    /**
     * Initializes the collDealerMetaSeos collection.
     *
     * By default this just sets the collDealerMetaSeos collection to an empty array (like clearcollDealerMetaSeos());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initDealerMetaSeos($overrideExisting = true)
    {
        if (null !== $this->collDealerMetaSeos && !$overrideExisting) {
            return;
        }
        $this->collDealerMetaSeos = new ObjectCollection();
        $this->collDealerMetaSeos->setModel('\Dealer\Model\DealerMetaSeo');
    }

    /**
     * Gets an array of ChildDealerMetaSeo objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return Collection|ChildDealerMetaSeo[] List of ChildDealerMetaSeo objects
     * @throws PropelException
     */
    public function getDealerMetaSeos($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collDealerMetaSeosPartial && !$this->isNew();
        if (null === $this->collDealerMetaSeos || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collDealerMetaSeos) {
                // return empty collection
                $this->initDealerMetaSeos();
            } else {
                $collDealerMetaSeos = ChildDealerMetaSeoQuery::create(null, $criteria)
                    ->filterByDealer($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collDealerMetaSeosPartial && count($collDealerMetaSeos)) {
                        $this->initDealerMetaSeos(false);

                        foreach ($collDealerMetaSeos as $obj) {
                            if (false == $this->collDealerMetaSeos->contains($obj)) {
                                $this->collDealerMetaSeos->append($obj);
                            }
                        }

                        $this->collDealerMetaSeosPartial = true;
                    }

                    reset($collDealerMetaSeos);

                    return $collDealerMetaSeos;
                }

                if ($partial && $this->collDealerMetaSeos) {
                    foreach ($this->collDealerMetaSeos as $obj) {
                        if ($obj->isNew()) {
                            $collDealerMetaSeos[] = $obj;
                        }
                    }
                }

                $this->collDealerMetaSeos = $collDealerMetaSeos;
                $this->collDealerMetaSeosPartial = false;
            }
        }

        return $this->collDealerMetaSeos;
    }

    /**
     * Sets a collection of DealerMetaSeo objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $dealerMetaSeos A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildDealer The current object (for fluent API support)
     */
    public function setDealerMetaSeos(Collection $dealerMetaSeos, ConnectionInterface $con = null)
    {
        $dealerMetaSeosToDelete = $this->getDealerMetaSeos(new Criteria(), $con)->diff($dealerMetaSeos);


        $this->dealerMetaSeosScheduledForDeletion = $dealerMetaSeosToDelete;

        foreach ($dealerMetaSeosToDelete as $dealerMetaSeoRemoved) {
            $dealerMetaSeoRemoved->setDealer(null);
        }

        $this->collDealerMetaSeos = null;
        foreach ($dealerMetaSeos as $dealerMetaSeo) {
            $this->addDealerMetaSeo($dealerMetaSeo);
        }

        $this->collDealerMetaSeos = $dealerMetaSeos;
        $this->collDealerMetaSeosPartial = false;
    ...
    /**
     * Returns the number of related DealerMetaSeo objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related DealerMetaSeo objects.
     * @throws PropelException
     */
    public function countDealerMetaSeos(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collDealerMetaSeosPartial && !$this->isNew();
        if (null === $this->collDealerMetaSeos || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collDealerMetaSeos) {
            ...
            if ($partial && !$criteria) {
                return count($this->getDealerMetaSeos());
            }

            $query = ChildDealerMetaSeoQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collDealerMetaSeos);
    }

    /**
     * Method called to associate a ChildDealerMetaSeo object to this object
     * through the ChildDealerMetaSeo foreign key attribute.
     *
     * @param    ChildDealerMetaSeo $l ChildDealerMetaSeo
     * @return   \Dealer\Model\Dealer The current object (for fluent API support)
     */
    public function addDealerMetaSeo(ChildDealerMetaSeo $l)
    {
        if ($this->collDealerMetaSeos === null) {
            $this->initDealerMetaSeos();
            $this->collDealerMetaSeosPartial = true;
        ...
        if (!in_array($l, $this->collDealerMetaSeos->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddDealerMetaSeo($l);
        }
    ...
    /**
     * @param DealerMetaSeo $dealerMetaSeo The dealerMetaSeo object to add.
     */
    protected function doAddDealerMetaSeo($dealerMetaSeo)
    {
        $this->collDealerMetaSeos[]= $dealerMetaSeo;
        $dealerMetaSeo->setDealer($this);
    }

    /**
     * @param  DealerMetaSeo $dealerMetaSeo The dealerMetaSeo object to remove.
     * @return ChildDealer The current object (for fluent API support)
     */
    public function removeDealerMetaSeo($dealerMetaSeo)
    {
        if ($this->getDealerMetaSeos()->contains($dealerMetaSeo)) {
            $this->collDealerMetaSeos->remove($this->collDealerMetaSeos->search($dealerMetaSeo));
            if (null === $this->dealerMetaSeosScheduledForDeletion) {
                $this->dealerMetaSeosScheduledForDeletion = clone $this->collDealerMetaSeos;
                $this->dealerMetaSeosScheduledForDeletion->clear();
            }
            $this->dealerMetaSeosScheduledForDeletion[]= clone $dealerMetaSeo;
            $dealerMetaSeo->setDealer(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collDealerImages) {
                foreach ($this->collDealerImages as $o) {
                    $o->clearAllReferences($deep);
                }
            }
            if ($this->collDealerMetaSeos) {
                foreach ($this->collDealerMetaSeos as $o) {
                    $o->clearAllReferences($deep);
                }
            }
    ...
    public function getCurrentTranslation(ConnectionInterface $con = null)
    {
        ...
        /**
         * Get the [big_description] column value.
         *
         * @return   string
         */
        public function getBigDescription()
        {
        return $this->getCurrentTranslation()->getBigDescription();
    }


        /**
         * Set the value of [big_description] column.
         *
         * @param      string $v new value
         * @return   \Dealer\Model\DealerI18n The current object (for fluent API support)
         */
        public function setBigDescription($v)
        {    $this->getCurrentTranslation()->setBigDescription($v);
        ...
        /**
         * Get the [hard_open_hour] column value.
         *
         * @return   string
         */
        public function getHardOpenHour()
        {
        return $this->getCurrentTranslation()->getHardOpenHour();
    }


        /**
         * Set the value of [hard_open_hour] column.
         *
         * @param      string $v new value
         * @return   \Dealer\Model\DealerI18n The current object (for fluent API support)
         */
        public function setHardOpenHour($v)
        {    $this->getCurrentTranslation()->setHardOpenHour($v);
